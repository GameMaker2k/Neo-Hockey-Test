...
/*
    ...
    $FileInfo: xml.php - Last Update: 12/03/2023 Ver. 0.8.8 RC 1 - Author: cooldude2k $
*/
...
if (!isset($_GET['act']) && isset($_GET['view'])) {
    $_GET['act'] = "view";
}
if (!isset($_GET['act']) && isset($_GET['games'])) {
    $_GET['act'] = "view";
}
if (!isset($_GET['act']) && isset($_GET['stats'])) {
    $_GET['act'] = "stats";
}
if (!isset($_GET['act']) && isset($_GET['calendar'])) {
    $_GET['act'] = "calendar";
}
if (!isset($_GET['order']) && isset($_GET['asc'])) {
    $_GET['order'] = "ascending";
}
if (isset($_GET['order']) && $_GET['order'] == "asc") {
    $_GET['order'] = "ascending";
}
if (!isset($_GET['order']) && isset($_GET['ascending'])) {
    $_GET['order'] = "ascending";
}
...
if (!isset($_GET['order']) && isset($_GET['descending'])) {
    $_GET['order'] = "descending";
}
...
if (isset($_GET['date']) && strlen($_GET['date']) == 8) {
    ...
    if ($chckmonth > 12) {
        $chckmonth = $chckmonth = "12";
    }
    if ($chckmonth < 1) {
        $chckmonth = $chckmonth = "01";
    }
    $chckday = substr($_GET['date'], 6, 2);
    if ($chckday > gmdate("t", gmmktime(0, 0, 0, $chckmonth, 1, $chckyear))) {
        $chckday = gmdate("t", gmmktime(0, 0, 0, $chckmonth, 1, $chckyear));
    }
    if ($chckday < 1) {
        $chckday = "01";
    }
...
if ((isset($_GET['day']) && strlen($_GET['day']) == 2)) {
    if ((isset($_GET['year']) && strlen($_GET['year']) == 4)) {
        if ((isset($_GET['month']) && strlen($_GET['month']) == 2)) {
            ...
            if ($_GET['day'] > gmdate("t", gmmktime(0, 0, 0, $_GET['month'], 1, $_GET['year']))) {
                $_GET['day'] = gmdate("t", gmmktime(0, 0, 0, $_GET['month'], 1, $_GET['year']));
            }
        }
        if ((!isset($_GET['month']) || strlen($_GET['month']) != 2)) {
            $_GET['month'] = gmdate("m");
...
if ($_GET['act'] == "view") {
    ...
    while ($row = $results->fetchArray()) {
        ...
        if ($row['NumberPeriods'] == 3) {
            ...
            $tonebold = "";
            if ($PerTeamScoreTotal[0] > $PerTeamScoreTotal[1]) {
                $tonebold = " font-weight: bold;";
            }
            $ttwobold = "";
            if ($PerTeamScoreTotal[0] < $PerTeamScoreTotal[1]) {
                $ttwobold = " font-weight: bold;";
            }
...
if ($_GET['act'] == "stats") {
    ...
    while ($trow = $tresults->fetchArray()) {
        if ($trow['Shootouts'] == "0:0") {
            $trow['Shootouts'] = "-";
        }
        if ($trow['GoalsDifference'] == "0") {
            $trow['GoalsDifference'] = "E";
        }

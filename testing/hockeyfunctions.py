...
import sqlite3, sys, os, re, time, pickle, marshal, platform, tempfile, binascii, xml.dom.minidom;
...
def MakeFileFromString(instringfile, stringisfile, outstringfile, returnstring=False):
 ...
 if(re.findall("^(http|https)\:\/\/", outstringfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(stringfp, outstringfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outstringfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(stringfp, outstringfile);
  ftpfilefp.close();
 stringfp.close();
 if(returnstring):
  return stringstring;
...
def BeautifyXMLCodeToFile(inxmlfile, outxmlfile, xmlisfile=True, indent="\t", newl="\n", encoding="UTF-8", beautify=True, returnxml=False):
 ...
 if(re.findall("^(http|https)\:\/\/", outxmlfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(xmlfp, outxmlfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outxmlfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(xmlfp, outxmlfile);
  ftpfilefp.close();
 xmlfp.close();
 if(returnxml):
  return xmlstring;
...
def MakeHockeyJSONFileFromHockeyArray(inhockeyarray, outjsonfile=None, returnjson=False, jsonindent=1, beautify=True, sortkeys=False, verbose=True, jsonverbose=True):
 ...
 if(re.findall("^(http|https)\:\/\/", outjsonfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(jsonfp, outjsonfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outjsonfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(jsonfp, outjsonfile);
  ftpfilefp.close();
 jsonfp.close();
 if(returnjson):
  return jsonstring;
...
def MakeHockeyPickleFileFromHockeyArray(inhockeyarray, outpicklefile=None, returnpickle=False, protocol=pickledp, verbose=True, jsonverbose=True):
 ...
 if(re.findall("^(http|https)\:\/\/", outpicklefile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(picklefp, outpicklefile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outpicklefile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(picklefp, outpicklefile);
  ftpfilefp.close();
 picklefp.close();
 if(returnpickle):
  return picklestring;
...
def MakeHockeyMarshalFileFromHockeyArray(inhockeyarray, outmarshalfile=None, returnmarshal=False, version=marshal.version, verbose=True, jsonverbose=True):
 ...
 if(re.findall("^(http|https)\:\/\/", outmarshalfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(marshalfp, outmarshalfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outmarshalfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(marshalfp, outmarshalfile);
  ftpfilefp.close();
 marshalfp.close();
 if(returnmarshal):
  return marshalstring;
...
def MakeHockeyPythonFileFromHockeyArray(inhockeyarray, outpyfile=None, returnpy=False, verbose=True, jsonverbose=True):
 ...
 if(re.findall("^(http|https)\:\/\/", outpyfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(pyfp, outpyfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outpyfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(pyfp, outpyfile);
  ftpfilefp.close();
 pyfp.close();
 if(fextname not in outextlistwd):
...
def MakeHockeySQLFileFromHockeyArray(inhockeyarray, outsqlfile=None, returnsql=False, verbose=True, jsonverbose=True):
 ...
 if(re.findall("^(http|https)\:\/\/", outsqlfile)):
  ...
  ftpfilefp = upload_file_to_ftp_file(sqlfp, outsqlfile);
  ftpfilefp.close();
 elif(re.findall("^(sftp)\:\/\/", outsqlfile) and testparamiko):
  sftpfilefp = upload_file_to_sftp_file(sqlfp, outsqlfile);
  ftpfilefp.close();
 sqlfp.close();
 if(returnsql):
  return sqlstring;

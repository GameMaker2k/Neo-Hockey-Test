...
if (not getargs.empty and getargs.infile is None):
    ...
    if (premenuact.upper() != "E" and not premenuact.isdigit()):
        ...
        premenuact = "E"
    if (premenuact.upper() != "E" and premenuact.isdigit()
            and (int(premenuact) > 2 or int(premenuact) < 1)):
        print("ERROR: Invalid Command")
        premenuact = "E"
...
if (premenuact == "2"):
    ...
    if (ext in extensionsin or subext in extensions):
        ...
        if (getargs.export):
            verbosein = False
        if ((ext == ".xml" or subext == ".xml") and libhockeydata.CheckXMLFile(
                HockeyDatabaseFN) and libhockeydata.CheckHockeyXML(HockeyDatabaseFN)):
            hockeyarray = libhockeydata.MakeHockeyArrayFromHockeyXML(
                HockeyDatabaseFN, verbose=verbosein, jsonverbose=getargs.jsonverbose)
        if ((ext == ".xml" or subext == ".xml") and libhockeydata.CheckXMLFile(
                HockeyDatabaseFN) and libhockeydata.CheckHockeySQLiteXML(HockeyDatabaseFN)):
...
while (keep_loop):
    ...
    if (menuact.upper() != "E" and not menuact.isdigit()):
        print("ERROR: Invalid Command")
        menuact = ""
    if (menuact.upper() != "E" and menuact.isdigit()
            and (int(menuact) > 7 or int(menuact) < 1)):
        print("ERROR: Invalid Command")
        menuact = ""
    if (menuact == "1"):
        sub_keep_loop = True
        while (sub_keep_loop):
            ...
            if (submenuact.upper() != "E" and not submenuact.isdigit()):
                print("ERROR: Invalid Command")
                submenuact = ""
            if (submenuact.upper() != "E" and submenuact.isdigit()
                    and (int(submenuact) > 3 or int(submenuact) < 1)):
                print("ERROR: Invalid Command")
                submenuact = ""
            if (submenuact == "1"):
            ...
            if (submenuact == "2" and len(hockeyarray['leaguelist']) > 0):
                ...
                if (HockeyLeaguePreSN.upper() !=
                        "E" and not HockeyLeaguePreSN.isdigit()):
                    print("ERROR: Invalid Command")
                    HockeyLeaguePreSN = "E"
                if (HockeyLeaguePreSN.upper() != "E" and HockeyLeaguePreSN.isdigit() and (int(
                        HockeyLeaguePreSN) > len(hockeyarray['leaguelist']) or int(HockeyLeaguePreSN) < 0)):
                    print("ERROR: Invalid Command")
                    HockeyLeaguePreSN = "E"
                if (HockeyLeaguePreSN.upper() != "E" and int(HockeyLeaguePreSN) < len(
                        hockeyarray['leaguelist']) and int(HockeyLeaguePreSN) > -1):
    ...
    if (menuact == "2" and len(hockeyarray['leaguelist']) > 0):
        ...
        while (sub_keep_loop):
            ...
            if (HockeyLeaguePreSN.upper() != "E" and int(HockeyLeaguePreSN) < len(
                ...
                HockeyLeagueIntSN = int(HockeyLeaguePreSN)
                HockeyLeagueSN = hockeyarray['leaguelist'][HockeyLeagueIntSN]
                if (hockeyarray[HockeyLeagueSN]
                        ['leagueinfo']['conferences'] == "no"):
                    print("ERROR: Hockey League can not have any conferences")
                    HockeyLeaguePreSN = "E"
                if (hockeyarray[HockeyLeagueSN]
                        ['leagueinfo']['conferences'] == "yes"):
                    ...
                    while (sub_sub_keep_loop):
                        ...
                        if (subsubmenuact.upper() !=
                                "E" and not subsubmenuact.isdigit()):
                            print("ERROR: Invalid Command")
                            subsubmenuact = ""
                        if (subsubmenuact.upper() != "E" and subsubmenuact.isdigit() and (
                                int(subsubmenuact) > 3 or int(subsubmenuact) < 1)):
                            print("ERROR: Invalid Command")
                            subsubmenuact = ""
                        if (subsubmenuact.upper() == "1"):
                            ...
                            if (
                                    HockeyConferenceCN in hockeyarray[HockeyLeagueSN]['conferencelist']):
                                print(
                                    "ERROR: Hockey Conference with that name exists")
                            if (
                                    HockeyConferenceCN not in hockeyarray[HockeyLeagueSN]['conferencelist']):
                                ...
                                hockeyarray = libhockeydata.AddHockeyConferenceToArray(
                                    ...
                                    HockeyLeagueSN,
                                    HockeyConferenceCN,
                                    HockeyConferenceCPFN,
                                    HockeyConferenceCSFN)
                        if (subsubmenuact == "2" and (len(hockeyarray['leaguelist']) <= 0 or len(
                                hockeyarray[HockeyLeagueSN]['conferencelist']) <= 0)):
                            print("ERROR: There are no Hockey Conferences to delete")
                        if (subsubmenuact == "2" and len(hockeyarray['leaguelist']) > 0 and len(
                                hockeyarray[HockeyLeagueSN]['conferencelist']) > 0):
                            conferencec = 0
                            print("E: Back to Hockey Conference Tool")
                            while (
                                conferencec < len(
                                    hockeyarray[HockeyLeagueSN]['conferencelist'])):
                                lshn = hockeyarray[HockeyLeagueSN]['conferencelist'][conferencec]
                                print(
                                    str(conferencec) +
                                    ": " +
                                    hockeyarray[HockeyLeagueSN]['quickinfo']['conferenceinfo'][lshn]['fullname'])
                                conferencec = conferencec + 1
                            HockeyConferencePreCN = get_user_input(
                                "Enter Hockey Conference number: ")
                            if (HockeyConferencePreCN.upper() !=
                                    "E" and not HockeyConferencePreCN.isdigit()):
                                print("ERROR: Invalid Command")
                                HockeyConferencePreCN = "E"
                            if (HockeyConferencePreCN.upper() != "E" and HockeyConferencePreCN.isdigit(
                            ) and (int(HockeyConferencePreCN) > 6 or int(HockeyConferencePreCN) < 0)):
                                print("ERROR: Invalid Command")
                                HockeyConferencePreCN = "E"
                            if (HockeyConferencePreCN.upper() != "E" and int(HockeyConferencePreCN) < len(
                                    hockeyarray[HockeyLeagueSN]['conferencelist']) and int(HockeyConferencePreCN) > -1):
                        ...
                        if (subsubmenuact == "3" and (len(hockeyarray['leaguelist']) <= 0 or len(
                                hockeyarray[HockeyLeagueSN]['conferencelist']) <= 0)):
                            print("ERROR: There are no Hockey Conferences to edit")
                        if (subsubmenuact == "3" and len(hockeyarray['leaguelist']) > 0 and len(
                                hockeyarray[HockeyLeagueSN]['conferencelist']) > 0):
                            conferencec = 0
                            print("E: Back to Hockey Conference Tool")
                            while (
                ...
                if (hockeyarray[HockeyLeagueSN]
                    ...
                    if (hockeyarray[HockeyLeagueSN]
                        ...
                        while (sub_sub_keep_loop):
                            ...
                            if (submenuact.upper() !=
                                    "E" and not submenuact.isdigit()):
                                print("ERROR: Invalid Command")
                                submenuact = ""
                            if (submenuact.upper() != "E" and submenuact.isdigit() and (
                                    int(submenuact) > 3 or int(submenuact) < 1)):
                                print("ERROR: Invalid Command")
                                submenuact = ""
                            if (submenuact.upper() == "1"):
                                ...
                                if (
                                        HockeyDivisionDN in hockeyarray[HockeyLeagueSN]['']['divisionlist']):
                                    print(
                                        "ERROR: Hockey Division with that name exists")
                                if (
                                        HockeyDivisionDN not in hockeyarray[HockeyLeagueSN]['']['divisionlist']):
                            ...
                            if (submenuact.upper() == "2"):
                                ...
                                while (
                                    divisionc < len(
                                        hockeyarray[HockeyLeagueSN]['']['divisionlist'])):
                                    lshn = hockeyarray[HockeyLeagueSN]['']['divisionlist'][divisionc]
                                    print(
                                        str(divisionc) +
                                        ": " +
                                        hockeyarray[HockeyLeagueSN]['quickinfo']['divisioninfo'][lshn]['fullname'])
                                    divisionc = divisionc + 1
                                HockeyDivisionPreDN = get_user_input(
                                    "Enter Hockey Division number: ")
                                if (HockeyDivisionPreDN.upper() !=
                                        "E" and not HockeyDivisionPreDN.isdigit()):
                                    print("ERROR: Invalid Command")
                                    HockeyDivisionPreDN = "E"
                                if (HockeyDivisionPreDN.upper() != "E" and HockeyDivisionPreDN.isdigit() and (
                                        int(HockeyDivisionPreDN) > 6 or int(HockeyDivisionPreDN) < 0)):
                                    print("ERROR: Invalid Command")
                                    HockeyDivisionPreDN = "E"
                                if (HockeyDivisionPreDN.upper() != "E" and int(HockeyDivisionPreDN) < len(
                                        hockeyarray[HockeyLeagueSN]['']['divisionlist']) and int(HockeyDivisionPreDN) > -1):
                    ...
                    if (hockeyarray[HockeyLeagueSN]
                        ...
                        while (sub_sub_keep_loop):
                            ...
                            HockeyConferencePreSN = get_user_input(
                                "Enter Hockey Conference number: ")
                            if (HockeyConferencePreSN.upper() !=
                                    "E" and not HockeyConferencePreSN.isdigit()):
                                print("ERROR: Invalid Command")
                                HockeyConferencePreSN = "E"
                            if (
                                HockeyConferencePreSN.upper() != "E" and HockeyConferencePreSN.isdigit() and (
                                    int(HockeyConferencePreSN) > len(
                                        hockeyarray[HockeyLeagueSN]['conferencelist']) or int(HockeyConferencePreSN) < 0)):
                                print("ERROR: Invalid Command")
                                HockeyConferencePreSN = "E"
                            if (HockeyConferencePreSN.upper() != "E" and int(HockeyConferencePreSN) < len(
                                    hockeyarray[HockeyLeagueSN]['conferencelist']) and int(HockeyConferencePreSN) > -1):
                                ...
                                while (sub_sub_sub_keep_loop):
                                    ...
                                    if (submenuact.upper() == "1"):
                                        HockeyDivisionDN = get_user_input(
                                            "Enter Hockey Division name: ")
                                        if (
                                                HockeyDivisionDN in hockeyarray[HockeyLeagueSN][HockeyConferenceSN]['divisionlist']):
                                            print(
                                                "ERROR: Hockey Division with that name exists")
                                        if (
                                                HockeyDivisionDN not in hockeyarray[HockeyLeagueSN][HockeyConferenceSN]['divisionlist']):
                                            HockeyDivisionDPFN = get_user_input(
                                                "Enter Hockey Division prefix: ")
                                            HockeyDivisionDSFN = get_user_input(
                                                "Enter Hockey Division suffix: ")
                                    ...
                                    if (submenuact.upper() == "2"):
                                        ...
                                        while (
                                            divisionc < len(
                                                hockeyarray[HockeyLeagueSN][HockeyConferenceSN]['divisionlist'])):
                                        ...
                                        if (HockeyDivisionPreDN.upper() != "E" and HockeyDivisionPreDN.isdigit() and (
                                            ...
                                            HockeyDivisionPreDN = "E"
                                        if (HockeyDivisionPreDN.upper() != "E" and int(HockeyDivisionPreDN) < len(
                                                hockeyarray[HockeyLeagueSN][HockeyConferenceSN]['divisionlist']) and int(HockeyDivisionPreDN) > -1):
                                            HockeyDivisionIntCN = int(
                                                HockeyDivisionPreDN)
    ...
    if (menuact == "7"):
        ...
        while (sub_keep_loop):
            ...
            if (submenuact == "2"):
                ...
                if (ext in extensions):
                    if (ext == ".xml" and libhockeydata.CheckXMLFile(
                            HockeyDatabaseFN)):
                        hockeyarray = libhockeydata.MakeHockeyArrayFromHockeyXML(
                            HockeyDatabaseFN)
                    elif (ext == ".db3" and libhockeydata.CheckSQLiteDatabase(HockeyDatabaseFN)):
            ...
            if (submenuact.upper() == "3"):
                ...
                while (sub_sub_keep_loop):
                    ...
                    if (subsubmenuact.upper() !=
                        ...
                        subsubmenuact = "E"
                    if (subsubmenuact.upper() != "E" and subsubmenuact.isdigit() and (
                            int(subsubmenuact) > 6 or int(subsubmenuact) < 1)):
                        print("ERROR: Invalid Command")
                        subsubmenuact = "E"
                    if (subsubmenuact == "1"):
                        HockeyDatabaseFN = get_user_input(

...
'''
    ...
    $FileInfo: hockeyoopfuncs.py - Last Update: 12/03/2023 Ver. 0.8.8 RC 1 - Author: cooldude2k $
'''
...
class MakeHockeyArray:
    ...
    def MoveHockeyDivisionToConference(
            self,
            leaguename,
            division,
            oldconference,
    ...
    def MoveHockeyTeamToConference(
            self,
            leaguename,
            teamname,
            oldconference,
    ...
    def MakeHockeyPythonAlt(self, verbose=True, verbosepy=True):
        return MakeHockeyPythonAltFromHockeyArray(
            self.hockeyarray, verbose, verbosepy)

    def MakeHockeySQL(self, verbose=True):
        return MakeHockeySQLFromHockeyArray(self.hockeyarray, verbose)

    def MakeHockeyXMLFile(
            self,
            outxmlfile=None,
            returnxml=False,
            verbose=True):
        return MakeHockeyXMLFileFromHockeyArray(
            self.hockeyarray, outxmlfile, returnxml, verbose)

    def MakeHockeyPythonFile(
            self,
            outpyfile=None,
            returnpy=False,
            verbose=True):
        return MakeHockeyPythonFileFromHockeyArray(
            self.hockeyarray, outpyfile, returnpy, verbose)
    ...
    def MakeHockeySQLFile(self, sqlfile=None, returnsql=False, verbose=True):
        return MakeHockeySQLFileFromHockeyArray(
            self.hockeyarray, sqlfile, returnsql, verbose)

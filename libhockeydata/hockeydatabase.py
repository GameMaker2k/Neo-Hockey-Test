...
def check_version_number(
        ...
        proname=__program_alt_name__,
        newverurl=__project_release_url__):
    prevercheck = download_from_url(newverurl, geturls_headers, geturls_cj)
    newvercheck = re.findall(
        proname + r" ([0-9\.]+)<\/a\>",
        prevercheck['Content'].decode("UTF-8"))[0]
...
def CheckHockeySQLiteDatabaseFile(sdbfile):
    if (os.path.exists(sdbfile) and os.path.isfile(
            sdbfile) and isinstance(sdbfile, basestring)):
...
def CheckHockeySQLiteDatabase(sdbfile, returndb=False):
    ...
    if (sqldatacur.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='HockeyLeagues';").fetchone() is None):
        return [False]
...
def MakeHockeyDatabase(
        sdbfile,
        synchronous=sqlite_synchronous,
        journal_mode=sqlite_journal_mode,
        journal_size=sqlite_journal_size,
        mmap_size=sqlite_mmap_size,
        temp_store=sqlite_temp_store,
        enable_oldsqlite=enable_oldsqlite,
        enable_apsw=enable_apsw,
        enable_supersqlite=enable_supersqlite,
        enable_sqlcipher=enable_sqlcipher,
        sqlite_password=sqlcipher_password):
    usecipher = False
...
def GetLastGames(sqldatacon, leaguename, teamname, gamelimit=10):
    ...
    getlastninegames = sqldatacon[0].execute(
        "SELECT NumberPeriods, TeamWin, TeamLost, TieGame FROM " +
        leaguename +
        ...
        str(gamelimit)).fetchall()
    nmax = len(getlastninegames)
    nmin = 0
    while (nmin < nmax):
        if (teamname == str(getlastninegames[nmin][1]) and int(
                getlastninegames[nmin][3]) == 0):
            wins = wins + 1
        if (teamname == str(getlastninegames[nmin][2]) or int(
                getlastninegames[nmin][3]) == 1):
...
def GetLastGamesWithShootout(sqldatacon, leaguename, teamname, gamelimit=10):
    ...
    solosses = 0
    getlastninegames = sqldatacon[0].execute(
        "SELECT NumberPeriods, TeamWin, TeamLost, TieGame, IsPlayOffGame FROM " +
        leaguename +
        "Games WHERE (HomeTeam=\"" +
        str(teamname) +
        "\" OR AwayTeam=\"" +
    ...
    while (nmin < nmax):
        ...
        if (teamname == str(getlastninegames[nmin][2]) or int(
            ...
            if (int(getlastninegames[nmin][0]) == 3):
                losses = losses + 1
            if (int(getlastninegames[nmin][0]) == 4 and int(
                    getlastninegames[nmin][4]) == 0):
                otlosses = otlosses + 1
            if (int(getlastninegames[nmin][0]) > 4 and int(
                    getlastninegames[nmin][4]) == 0):
                solosses = solosses + 1
            if (int(getlastninegames[nmin][0]) > 3 and (
                    int(getlastninegames[nmin][4]) == 1 or int(getlastninegames[nmin][4]) == 2)):
                otlosses = otlosses + 1
        nmin = nmin + 1
    return str(wins) + ":" + str(losses) + ":" + \
        str(otlosses) + ":" + str(solosses)
...
def GetLastGamesWithoutShootout(
        sqldatacon,
        leaguename,
        teamname,
        gamelimit=10):
...
def GetLeagueName(sqldatacon, leaguename):
    ...
    TMPData = str(
        sqldatacon[0].execute(
            "SELECT LeagueFullName FROM HockeyLeagues WHERE LeagueName=\"" +
            str(leaguename) +
            "\"").fetchone()[0])
    return TMPData
...
def GetAreaInfoFromUSCA(areaname):
    ...
    return areacodes.get(areaname,
                         {areaname: {'AreaName': areaname,
                                     'FullAreaName': "Unknown",
                                     'CountryName': "Unknown",
                                     'FullCountryName': "Unknown"}})
...
def GetHockeyLeaguesInfo(leaguename):
    ...
    leagueinfo = {
        'NHL': {
            'LeagueName': "NHL",
            'FullLeagueName': "National Hockey League",
            'CountryName': "USA",
            'FullCountryName': "United States",
            'StartDate': 20151007,
            'PlayOffFMT': "Division=3,Conference=2",
        ...
        'AHL': {
            ...
            'FullCountryName': "United States",
            'StartDate': 20151009,
            'PlayOffFMT': "Division=4",
            'OrderType': "ORDER BY PCT DESC, GamesPlayed ASC, TWins DESC, Losses ASC, GoalsDifference DESC"},
        'ECHL': {
            'LeagueName': "ECHL",
            'FullLeagueName': "ECHL",
            'CountryName': "USA",
            'FullCountryName': "United States",
            'StartDate': 20151007,
            'PlayOffFMT': "Division=1,Conference=5",
            'OrderType': "ORDER BY PCT DESC, GamesPlayed ASC, TWins DESC, Losses ASC, GoalsDifference DESC"},
        'FHL': {
            'LeagueName': "FHL",
            'FullLeagueName': "Federal Hockey League",
            'CountryName': "USA",
            'FullCountryName': "United States",
            'StartDate': 20151106,
            'PlayOffFMT': "League=4",
        ...
        'SPHL': {
            'LeagueName': "SPHL",
            'FullLeagueName': "Southern Professional Hockey League",
            'CountryName': "USA",
            'FullCountryName': "United States",
            'StartDate': 20151023,
            'PlayOffFMT': "League=8",
            'OrderType': "ORDER BY Points DESC, GamesPlayed ASC, TWins DESC, Losses ASC, GoalsDifference DESC"}}
    return leagueinfo.get(leaguename,
                          {leaguename: {'LeagueName': leaguename,
                                        'FullLeagueName': "Unknown",
                                        'CountryName': "Unknown",
                                        'FullCountryName': "Unknown",
                                        'StartDate': 0,
                                        'PlayOffFMT': "Unknown",
...
def CheckHockeyArray(inhockeyarray):
    if (isinstance(inhockeyarray,
                   type(None)) or isinstance(inhockeyarray,
                                             type(True)) or isinstance(inhockeyarray,
                                                                       type(False)) or not isinstance(inhockeyarray,
                                                                                                      type({}))):
        return False
    ...
    for hlkey in inhockeyarray['leaguelist']:
        ...
        for hckey in inhockeyarray[hlkey]['conferencelist']:
            if hckey not in inhockeyarray[hlkey].keys(
            ) or hckey not in inhockeyarray[hlkey]['quickinfo']['conferenceinfo'].keys():
                return False
            for hdkey in inhockeyarray[hlkey][hckey]['divisionlist']:
                if hdkey not in inhockeyarray[hlkey][hckey].keys(
                ) or hdkey not in inhockeyarray[hlkey]['quickinfo']['divisioninfo'].keys():
                    return False
                for htkey in inhockeyarray[hlkey][hckey][hdkey]['teamlist']:
                    if htkey not in inhockeyarray[hlkey][hckey][hdkey].keys(
                    ) or htkey not in inhockeyarray[hlkey]['quickinfo']['teaminfo'].keys():
                        return False
    return True
...
def ReplaceHockeyLeagueFromArray(
        inhockeyarray,
        oldleaguename,
        newleaguename,
        leaguefullname=None,
        countryname=None,
        fullcountryname=None,
        date=None,
        playofffmt=None,
        ordertype=None,
        hasconferences=None,
    ...
    if "database" not in inchockeyarray.keys():
        inchockeyarray.update({'database': "./hockeydatabase.db3"})
    if oldleaguename in inchockeyarray.keys(
    ) and newleaguename not in inchockeyarray.keys():
...
def MakeHockeyLeague(
    ...
    if (not CheckHockeySQLiteDatabaseConnection(sqldatacon)):
        return False
    sqldatacon[0].execute(
        "INSERT INTO HockeyLeagues (LeagueName, LeagueFullName, CountryName, FullCountryName, Date, PlayOffFMT, OrderType, NumberOfTeams, NumberOfConferences, NumberOfDivisions) VALUES \n" +
        "(\"" +
        str(leaguename) +
        "\", \"" +
        str(leaguefullname) +
...
def AddHockeyConferenceToArray(
        inhockeyarray,
        leaguename,
        conference,
        prefix="",
    ...
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys(
    ) and "conferencelist" not in inchockeyarray[leaguename].keys():
    ...
    if leaguename in inchockeyarray.keys():
        if conference not in inchockeyarray[leaguename].keys():
            ...
            inchockeyarray[leaguename].update(
                {
                    str(conference): {
                        'conferenceinfo': {
                            'name': str(conference),
            ...
            for hdkey in inchockeyarray[leaguename][conference]['divisionlist']:
                ...
                for htkey in inchockeyarray[leaguename][conference][hdkey]['teamlist']:
                    ...
                    for hgkey in inchockeyarray[leaguename]['games']:
                        if (hgkey['hometeam'] !=
                                fullteamname and hgkey['awayteam'] != fullteamname):
...
def ReplaceHockeyConferencFromArray(
        inhockeyarray,
        leaguename,
        oldconference,
        newconference,
    ...
    if "database" not in inchockeyarray.keys():
        inchockeyarray.update({'database': "./hockeydatabase.db3"})
    if oldconference in inchockeyarray[leaguename].keys(
    ) and newconference not in inchockeyarray[leaguename].keys():
...
def MakeHockeyConference(
    ...
    sqldatacon[0].execute(
        "INSERT INTO " +
        leaguename +
        "Conferences (Conference, ConferencePrefix, ConferenceSuffix, FullName, LeagueName, LeagueFullName, NumberOfTeams, NumberOfDivisions) VALUES \n" +
        "(\"" +
        str(conference) +
        "\", \"" +
        str(prefix) +
...
def AddHockeyDivisionToArray(
        ...
        suffix="Division"):
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys() and conference in inchockeyarray[leaguename].keys(
    ) and "divisionlist" not in inchockeyarray[leaguename][conference].keys():
    ...
    if leaguename in inchockeyarray.keys():
        if conference in inchockeyarray[leaguename].keys():
            if division not in inchockeyarray[leaguename][conference].keys():
                ...
                inchockeyarray[leaguename][conference].update(
                    {
                        str(division): {
                            'divisioninfo': {
                                'name': str(division),
                            ...
                            'teamlist': []}})
                inchockeyarray[leaguename]['quickinfo']['divisioninfo'].update(
                    {
                        str(division): {
                            'name': str(division),
                            'fullname': str(DivisionFullName),
...
def RemoveHockeyDivisionFromArray(
        inhockeyarray,
        leaguename,
        division,
        conference):
...
def ReplaceHockeyDivisionFromArray(
        inhockeyarray,
        leaguename,
        olddivision,
        newdivision,
    ...
    if "database" not in inchockeyarray.keys():
        inchockeyarray.update({'database': "./hockeydatabase.db3"})
    if olddivision in inchockeyarray[leaguename][conference].keys(
    ) and newdivision not in inchockeyarray[leaguename][conference].keys():
...
def MoveHockeyDivisionToConferenceFromArray(
        inhockeyarray,
        leaguename,
        division,
        oldconference,
        newconference):
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys() and newconference in inchockeyarray[leaguename].keys() and oldconference in inchockeyarray[leaguename].keys(
    ) and division in inchockeyarray[leaguename][oldconference].keys() and division not in inchockeyarray[leaguename][newconference].keys():
...
def MakeHockeyDivision(
    ...
    sqldatacon[0].execute(
        "INSERT INTO " +
        leaguename +
        "Divisions (Division, DivisionPrefix, DivisionSuffix, FullName, Conference, ConferenceFullName, LeagueName, LeagueFullName, NumberOfTeams) VALUES \n" +
        "(\"" +
        str(division) +
        "\", \"" +
        str(prefix) +
...
def AddHockeyTeamToArray(
    ...
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys() and conference in inchockeyarray[leaguename].keys(
    ) and division in inchockeyarray[leaguename][conference].keys() and "teamlist" not in inchockeyarray[leaguename][conference][division].keys():
        ...
        if conference in inchockeyarray[leaguename].keys():
            if division in inchockeyarray[leaguename][conference].keys():
                if teamname not in inchockeyarray[leaguename][conference][division].keys(
                    ...
                    inchockeyarray[leaguename]['quickinfo']['teaminfo'].update(
                        {
                            str(teamname): {
                                'name': str(teamname),
                                'fullname': fullteamname,
        ...
        if conference in inchockeyarray[leaguename].keys():
            if division in inchockeyarray[leaguename][conference].keys():
                if teamname in inchockeyarray[leaguename][conference][division].keys(
                    ...
                    fullteamname = GetFullTeamName(
                        inchockeyarray[leaguename][conference][division][teamname]['teaminfo']['name'],
...
def ReplaceHockeyTeamFromArray(
        inhockeyarray,
        leaguename,
        oldteamname,
        newteamname,
    ...
    if "database" not in inchockeyarray.keys():
        inchockeyarray.update({'database': "./hockeydatabase.db3"})
    if oldteamname in inchockeyarray[leaguename][conference][division].keys(
    ) and newteamname not in inchockeyarray[leaguename][conference][division].keys():
        oldfullteamname = GetFullTeamName(
            inchockeyarray[leaguename][conference][division][oldteamname]['teaminfo']['name'],
        ...
        inchockeyarray[leaguename][conference][division]['teamlist'][htin] = str(
            newteamname)
        newfullteamname = GetFullTeamName(
            inchockeyarray[leaguename][conference][division][newteamname]['teaminfo']['name'],
...
def MoveHockeyTeamToConferenceFromArray(
        inhockeyarray,
        leaguename,
        teamname,
        oldconference,
        ...
        division):
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys() and newconference in inchockeyarray[leaguename].keys() and oldconference in inchockeyarray[leaguename].keys() and division in inchockeyarray[leaguename][oldconference].keys(
    ) and teamname in inchockeyarray[leaguename][oldconference][division].keys() and teamname not in inchockeyarray[leaguename][newconference][division].keys():
...
def MoveHockeyTeamToDivisionFromArray(
        inhockeyarray,
        leaguename,
        teamname,
        conference,
        ...
        newdivision):
    inchockeyarray = deepcopy(inhockeyarray)
    if leaguename in inchockeyarray.keys() and conference in inchockeyarray[leaguename].keys() and olddivision in inchockeyarray[leaguename][conference].keys() and newdivision in inchockeyarray[leaguename][conference].keys(
    ) and teamname in inchockeyarray[leaguename][conference][olddivision].keys() and teamname not in inchockeyarray[leaguename][conference][newdivision].keys():
...
def MakeHockeyTeam(
    ...
    sqldatacon[0].execute("INSERT INTO " +
                          leaguename +
                          "Arenas (TeamID, TeamName, TeamFullName, CityName, AreaName, CountryName, FullCountryName, FullCityName, FullAreaName, FullCityNameAlt, ArenaName, FullArenaName, GamesPlayed) VALUES \n" +
                          "(" +
                          str(TeamID) +
                          ", \"" +
                          str(teamname) +
        ...
        if (subsubplayoffspl[0] == "Conference"):
            ...
            for conferenceinfo in getconference:
                sqldatacon[0].execute(
                    ...
                    str(
                        conferenceinfo[0]) +
                    "\" AND NOT EXISTS(SELECT TeamID FROM " +
                    leaguename +
                    "PlayoffTeams WHERE " +
                    leaguename +
                    "PlayoffTeams.TeamID = " +
...
def MakeHockeyStandingsTable(sqldatacon, leaguename, date, droptable=True):
    ...
    sqldatacon[0].execute(
        "CREATE TEMP TABLE " +
        leaguename +
        "Standings AS SELECT * FROM " +
        leaguename +
        "Stats " +
        SelectWhere +
        " GROUP BY TeamID ORDER BY TeamID ASC, Date DESC")
    return True
...
def MakeHockeyGame(
    ...
    sqldatacon[0].execute(
        "INSERT INTO " +
        leaguename +
        "Games (Date, Time, DateTime, HomeTeam, AwayTeam, AtArena, TeamScorePeriods, TeamFullScore, ShotsOnGoal, FullShotsOnGoal, ShotsBlocked, FullShotsBlocked, PowerPlays, FullPowerPlays, ShortHanded, FullShortHanded, Penalties, FullPenalties, PenaltyMinutes, FullPenaltyMinutes, HitsPerPeriod, FullHitsPerPeriod, TakeAways, FullTakeAways, GiveAways, FullGiveAways, FaceoffWins, FullFaceoffWins, NumberPeriods, TeamWin, TeamLost, TieGame, IsPlayOffGame) VALUES \n" +
        "(" +
        str(date) +
        ", " +
        str(time) +
    ...
    hometeamupdatedict = {'Date': int(date),
                          'GamesPlayed': int(hometeaminfo[1]) + 1,
                          'GamesPlayedHome': int(hometeaminfo[2]) + 1,
                          'GamesPlayedAway': int(hometeaminfo[75]),
                          'GoalsFor': int(hometeaminfo[3]) + int(teamscores[0]),
                          'GoalsAgainst': int(hometeaminfo[4]) + int(teamscores[1]),
                          'GoalsDifference': int(hometeaminfo[5]) + int(int(teamscores[0]) - int(teamscores[1])),
                          'SOGFor': int(hometeaminfo[6]) + int(teamssog[0]),
                          'SOGAgainst': int(hometeaminfo[7]) + int(teamssog[1]),
                          'SOGDifference': int(hometeaminfo[8]) + int(int(teamssog[0]) - int(teamssog[1])),
                          'ShotsBlockedFor': int(hometeaminfo[9]) + int(hometsb),
                          'ShotsBlockedAgainst': int(hometeaminfo[10]) + int(awaytsb),
                          'ShotsBlockedDifference': int(hometeaminfo[11]) + int(int(hometsb) - int(awaytsb)),
                          'PPGFor': int(hometeaminfo[12]) + int(homeppg),
                          'PPGAgainst': int(hometeaminfo[13]) + int(awayppg),
                          'PPGDifference': int(hometeaminfo[14]) + int(int(homeppg) - int(awayppg)),
                          'SHGFor': int(hometeaminfo[15]) + int(homeshg),
                          'SHGAgainst': int(hometeaminfo[16]) + int(awayshg),
                          'SHGDifference': int(hometeaminfo[17]) + int(int(homeshg) - int(awayshg)),
                          'PenaltiesFor': int(hometeaminfo[18]) + int(awaypens),
                          'PenaltiesAgainst': int(hometeaminfo[19]) + int(homepens),
                          'PenaltiesDifference': int(hometeaminfo[20]) + int(int(awaypens) - int(homepens)),
                          'PIMFor': int(hometeaminfo[21]) + int(homepims),
                          'PIMAgainst': int(hometeaminfo[22]) + int(awaypims),
                          'PIMDifference': int(hometeaminfo[23]) + int(int(homepims) - int(awaypims)),
                          'HITSFor': int(hometeaminfo[24]) + int(homehits),
                          'HITSAgainst': int(hometeaminfo[25]) + int(awayhits),
                          'HITSDifference': int(hometeaminfo[26]) + int(int(homehits) - int(awayhits)),
                          'TakeAways': int(hometeaminfo[27]) + int(hometaws),
                          'GiveAways': int(hometeaminfo[28]) + int(awaytaws),
                          'TAGADifference': int(hometeaminfo[29]) + int(int(hometaws) - int(awaytaws)),
                          'FaceoffWins': int(hometeaminfo[30]) + int(homefows),
                          'FaceoffLosses': int(hometeaminfo[31]) + int(awayfows),
                          'FaceoffDifference': int(hometeaminfo[32]) + int(int(homefows) - int(awayfows))}
    awayteamupdatedict = {'Date': int(date),
                          'GamesPlayed': int(awayteaminfo[1]) + 1,
                          'GamesPlayedHome': int(awayteaminfo[2]),
                          'GamesPlayedAway': int(awayteaminfo[75]) + 1,
                          'GoalsFor': int(awayteaminfo[3]) + int(teamscores[1]),
                          'GoalsAgainst': int(awayteaminfo[4]) + int(teamscores[0]),
                          'GoalsDifference': int(awayteaminfo[5]) + int(int(teamscores[1]) - int(teamscores[0])),
                          'SOGFor': int(awayteaminfo[6]) + int(teamssog[1]),
                          'SOGAgainst': int(awayteaminfo[7]) + int(teamssog[0]),
                          'SOGDifference': int(awayteaminfo[8]) + int(int(teamssog[1]) - int(teamssog[0])),
                          'ShotsBlockedFor': int(awayteaminfo[9]) + int(awaytsb),
                          'ShotsBlockedAgainst': int(awayteaminfo[10]) + int(hometsb),
                          'ShotsBlockedDifference': int(awayteaminfo[11]) + int(int(awaytsb) - int(hometsb)),
                          'PPGFor': int(awayteaminfo[12]) + int(awayppg),
                          'PPGAgainst': int(awayteaminfo[13]) + int(homeppg),
                          'PPGDifference': int(awayteaminfo[14]) + int(int(awayppg) - int(homeppg)),
                          'SHGFor': int(awayteaminfo[15]) + int(awayshg),
                          'SHGAgainst': int(awayteaminfo[16]) + int(homeshg),
                          'SHGDifference': int(awayteaminfo[17]) + int(int(awayshg) - int(homeshg)),
                          'PenaltiesFor': int(awayteaminfo[18]) + int(awaypens),
                          'PenaltiesAgainst': int(awayteaminfo[19]) + int(homepens),
                          'PenaltiesDifference': int(awayteaminfo[20]) + int(int(awaypens) - int(homepens)),
                          'PIMFor': int(awayteaminfo[21]) + int(awaypims),
                          'PIMAgainst': int(awayteaminfo[22]) + int(homepims),
                          'PIMDifference': int(awayteaminfo[23]) + int(int(awaypims) - int(homepims)),
                          'HITSFor': int(awayteaminfo[24]) + int(awayhits),
                          'HITSAgainst': int(awayteaminfo[25]) + int(homehits),
                          'HITSDifference': int(awayteaminfo[26]) + int(int(awayhits) - int(homehits)),
                          'TakeAways': int(awayteaminfo[27]) + int(awaytaws),
                          'GiveAways': int(awayteaminfo[28]) + int(hometaws),
                          'TAGADifference': int(awayteaminfo[29]) + int(int(awaytaws) - int(hometaws)),
                          'FaceoffWins': int(awayteaminfo[30]) + int(awayfows),
                          'FaceoffLosses': int(awayteaminfo[31]) + int(homefows),
                          'FaceoffDifference': int(awayteaminfo[32]) + int(int(awayfows) - int(homefows))}
    ...
    if ((not isplayoffgame and numberofperiods < 5 and tiegame == 0)
            or (isplayoffgame and tiegame == 0)):
        if (hometeam == winningteam):
    ...
    if (numberofperiods > 3 and tiegame == 0):
        if ((numberofperiods == 4 and not isplayoffgame)
    ...
    HomeWinsPCTAlt = float(
        "%.3f" %
        float(
            float(
                hometeamupdatedict['TWins'] +
                HomeOTLossesPCT) /
            float(
                hometeamupdatedict['GamesPlayed'])))
    HomeWinsPCT = float(
        "%.3f" %
        float(
            hometeamupdatedict['Points'] /
            float(
                hometeamupdatedict['GamesPlayed'] *
                2)))
    AwayOTLossesPCT = float("%.2f" % float(
        float(0.5) * float(awayteamupdatedict['OTSOLosses'])))
    AwayWinsPCTAlt = float(
        "%.3f" %
        float(
            float(
                awayteamupdatedict['TWins'] +
                AwayOTLossesPCT) /
            float(
                awayteamupdatedict['GamesPlayed'])))
    AwayWinsPCT = float(
        "%.3f" %
        float(
            awayteamupdatedict['Points'] /
            float(
                awayteamupdatedict['GamesPlayed'] *
                2)))
    ...
    sqldatacon[0].execute("INSERT INTO " +
                          leaguename +
                          "Stats (TeamID, Date, Time, DateTime, FullName, CityName, TeamPrefix, TeamSuffix, AreaName, CountryName, FullCountryName, FullCityName, FullAreaName, FullCityNameAlt, TeamName, Conference, ConferenceFullName, Division, DivisionFullName, LeagueName, LeagueFullName, ArenaName, FullArenaName, Affiliates, GamesPlayed, GamesPlayedHome, GamesPlayedAway, Ties, Wins, OTWins, SOWins, OTSOWins, TWins, Losses, OTLosses, SOLosses, OTSOLosses, TLosses, ROW, ROT, ShutoutWins, ShutoutLosses, HomeRecord, AwayRecord, Shootouts, GoalsFor, GoalsAgainst, GoalsDifference, SOGFor, SOGAgainst, SOGDifference, ShotsBlockedFor, ShotsBlockedAgainst, ShotsBlockedDifference, PPGFor, PPGAgainst, PPGDifference, SHGFor, SHGAgainst, SHGDifference, PenaltiesFor, PenaltiesAgainst, PenaltiesDifference, PIMFor, PIMAgainst, PIMDifference, HITSFor, HITSAgainst, HITSDifference, TakeAways, GiveAways, TAGADifference, FaceoffWins, FaceoffLosses, FaceoffDifference, Points, PCT, LastTen, Streak) VALUES \n" +
                          "(" +
                          str(hometeam) +
                          ", " +
                          str(date) +
                          ...
                          str(hometeaminfo[74]) +
                          "\", " +
                          str(hometeamupdatedict['GamesPlayed']) +
                          ", " +
                          str(hometeamupdatedict['GamesPlayedHome']) +
                          ", " +
                          str(hometeamupdatedict['GamesPlayedAway']) +
                          ", " +
                          str(hometeamupdatedict['Ties']) +
                          ", " +
                          str(hometeamupdatedict['Wins']) +
                          ", " +
                          str(hometeamupdatedict['OTWins']) +
                          ", " +
                          str(hometeamupdatedict['SOWins']) +
                          ", " +
                          str(hometeamupdatedict['OTSOWins']) +
                          ", " +
                          str(hometeamupdatedict['TWins']) +
                          ", " +
                          str(hometeamupdatedict['Losses']) +
                          ", " +
                          str(hometeamupdatedict['OTLosses']) +
                          ", " +
                          str(hometeamupdatedict['SOLosses']) +
                          ", " +
                          str(hometeamupdatedict['OTSOLosses']) +
                          ", " +
                          str(hometeamupdatedict['TLosses']) +
                          ", " +
                          str(hometeamupdatedict['ROW']) +
                          ", " +
                          str(hometeamupdatedict['ROT']) +
                          ", " +
                          str(hometeamupdatedict['ShutoutWins']) +
                          ", " +
                          str(hometeamupdatedict['ShutoutLosses']) +
                          ", \"" +
                          str(hometeamupdatedict['HomeRecord']) +
                          "\", \"" +
                          str(hometeamupdatedict['AwayRecord']) +
                          "\", \"" +
                          str(hometeamupdatedict['Shootouts']) +
                          "\", " +
                          str(teamscores[0]) +
                          ", " +
                          str(teamscores[1]) +
                          ...
                          str(int(homefows) -
                              int(awayfows)) +
                          ", " +
                          str(hometeamupdatedict['Points']) +
                          ", " +
                          str(hometeamupdatedict['PCT']) +
                          ", \"" +
                          str(hometeamupdatedict['LastTen']) +
                          "\", \"" +
                          str(hometeamupdatedict['Streak']) +
                          "\")")
    sqldatacon[0].execute("INSERT INTO " +
                          leaguename +
                          "Stats (TeamID, Date, Time, DateTime, FullName, CityName, TeamPrefix, TeamSuffix, AreaName, CountryName, FullCountryName, FullCityName, FullAreaName, FullCityNameAlt, TeamName, Conference, ConferenceFullName, Division, DivisionFullName, LeagueName, LeagueFullName, ArenaName, FullArenaName, Affiliates, GamesPlayed, GamesPlayedHome, GamesPlayedAway, Ties, Wins, OTWins, SOWins, OTSOWins, TWins, Losses, OTLosses, SOLosses, OTSOLosses, TLosses, ROW, ROT, ShutoutWins, ShutoutLosses, HomeRecord, AwayRecord, Shootouts, GoalsFor, GoalsAgainst, GoalsDifference, SOGFor, SOGAgainst, SOGDifference, ShotsBlockedFor, ShotsBlockedAgainst, ShotsBlockedDifference, PPGFor, PPGAgainst, PPGDifference, SHGFor, SHGAgainst, SHGDifference, PenaltiesFor, PenaltiesAgainst, PenaltiesDifference, PIMFor, PIMAgainst, PIMDifference, HITSFor, HITSAgainst, HITSDifference, TakeAways, GiveAways, TAGADifference, FaceoffWins, FaceoffLosses, FaceoffDifference, Points, PCT, LastTen, Streak) VALUES \n" +
                          ...
                          str(awayteaminfo[74]) +
                          "\", " +
                          str(awayteamupdatedict['GamesPlayed']) +
                          ", " +
                          str(awayteamupdatedict['GamesPlayedHome']) +
                          ", " +
                          str(awayteamupdatedict['GamesPlayedAway']) +
                          ", " +
                          str(awayteamupdatedict['Ties']) +
                          ", " +
                          str(awayteamupdatedict['Wins']) +
                          ", " +
                          str(awayteamupdatedict['OTWins']) +
                          ", " +
                          str(awayteamupdatedict['SOWins']) +
                          ", " +
                          str(awayteamupdatedict['OTSOWins']) +
                          ", " +
                          str(awayteamupdatedict['TWins']) +
                          ", " +
                          str(awayteamupdatedict['Losses']) +
                          ", " +
                          str(awayteamupdatedict['OTLosses']) +
                          ", " +
                          str(awayteamupdatedict['SOLosses']) +
                          ", " +
                          str(awayteamupdatedict['OTSOLosses']) +
                          ", " +
                          str(awayteamupdatedict['TLosses']) +
                          ", " +
                          str(awayteamupdatedict['ROW']) +
                          ", " +
                          str(awayteamupdatedict['ROT']) +
                          ", " +
                          str(awayteamupdatedict['ShutoutWins']) +
                          ", " +
                          str(awayteamupdatedict['ShutoutLosses']) +
                          ", \"" +
                          str(awayteamupdatedict['HomeRecord']) +
                          "\", \"" +
                          str(awayteamupdatedict['AwayRecord']) +
                          "\", \"" +
                          str(awayteamupdatedict['Shootouts']) +
                          "\", " +
                          str(teamscores[1]) +
                          ", " +
                          str(teamscores[0]) +
                          ...
                          str(int(awayfows) -
                              int(homefows)) +
                          ", " +
                          str(awayteamupdatedict['Points']) +
                          ", " +
                          str(awayteamupdatedict['PCT']) +
                          ", \"" +
                          str(awayteamupdatedict['LastTen']) +
                          "\", \"" +
                          str(awayteamupdatedict['Streak']) +
                          "\")")
    return True


def MakeHockeyGameOld(
        sqldatacon,
        leaguename,
        date,
        time,
    ...
    UpdateTeamData(
        sqldatacon, leaguename, hometeam, "ShotsBlockedDifference", int(
    ...
    UpdateTeamData(
        sqldatacon, leaguename, hometeam, "PenaltiesDifference", int(
    ...
    UpdateTeamData(
        sqldatacon, leaguename, hometeam, "FaceoffDifference", int(
            int(homefows) - int(awayfows)), "+")
    ...
    UpdateTeamData(
        sqldatacon, leaguename, awayteam, "ShotsBlockedDifference", int(
    ...
    UpdateTeamData(
        sqldatacon, leaguename, awayteam, "PenaltiesDifference", int(
    ...
    UpdateTeamData(
        sqldatacon, leaguename, awayteam, "FaceoffDifference", int(
            int(awayfows) - int(homefows)), "+")
    if (tiegame == 1):
    ...
    if (winningteam == awayteam and int(teamscores[0]) == 0):
        ...
        UpdateTeamData(sqldatacon, leaguename, hometeam,
                       "ShutoutLosses", 1, "+")
    UpdateTeamDataString(
        sqldatacon,
        leaguename,
        hometeam,
        "LastTen",
        GetLastGamesWithShootout(
            sqldatacon,
            leaguename,
            winningteamname))
    UpdateTeamDataString(
    ...
    sqldatacon[0].execute("INSERT INTO " +
                          ...
                          str(str(date) +
                              str(time)) +
                          ", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "CityName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "TeamPrefix") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "TeamSuffix") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "AreaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "CountryName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullCountryName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullCityName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullAreaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullCityNameAlt") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "TeamName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "Conference") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "ConferenceFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "Division") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "DivisionFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "LeagueName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "LeagueFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "ArenaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "FullArenaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(hometeam), "Affiliates") +
                          "\", " +
                          str(teamscores[0]) +
                          ", " +
                          str(teamscores[1]) +
    ...
    sqldatacon[0].execute("INSERT INTO " +
                          ...
                          str(str(date) +
                              str(time)) +
                          ", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "CityName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "TeamPrefix") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "TeamSuffix") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "AreaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "CountryName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullCountryName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullCityName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullAreaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullCityNameAlt") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "TeamName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "Conference") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "ConferenceFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "Division") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "DivisionFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "LeagueName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "LeagueFullName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "ArenaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "FullArenaName") +
                          "\", \"" +
                          GetNum2Team(sqldatacon, leaguename, int(awayteam), "Affiliates") +
                          "\", " +
                          str(teamscores[1]) +
                          ", " +
                          str(teamscores[0]) +
